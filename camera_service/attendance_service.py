...
# Final tuned defaults for this project; keep env overrides available for future camera setups.
MARK_SIMILARITY_THRESHOLD = float(os.getenv("MARK_SIMILARITY_THRESHOLD", "0.50"))
MIN_TOP2_MARGIN = float(os.getenv("MIN_TOP2_MARGIN", "0.03"))
RELAXED_MARK_SIMILARITY_THRESHOLD = float(os.getenv("RELAXED_MARK_SIMILARITY_THRESHOLD", "0.45"))
HIGH_MARGIN_OVERRIDE = float(os.getenv("HIGH_MARGIN_OVERRIDE", "0.10"))
MODEL = "ArcFace"
...
FORCE_MARK_ON_DETECT = os.getenv("FORCE_MARK_ON_DETECT", "true").lower() == "true"
...
class FaceDatabase:
    ...
    def _to_float_embedding(self, embedding_value):
        """Safely convert embedding payload to numeric numpy array."""
        try:
    ...
    def load_students(self):
                ...
                for student in students_list:
                    ...
                    if roll:
                        ...
                        if "embedding" in student and student["embedding"]:
                            embedding = self._to_float_embedding(student["embedding"])
                            if embedding is not None:
                                self.embeddings[roll] = embedding
                            else:
        ...
        for roll, student_data in self.students.items():
            if "embedding" in student_data:
                embedding = self._to_float_embedding(student_data["embedding"])
                if embedding is not None:
                    self.embeddings[roll] = embedding

        logger.info(f"✅ Loaded {len(self.embeddings)} valid face embeddings")
...
class CameraAttendance:
    ...
    def detect_faces_in_frame(self, frame):
        ...
        try:
            ...
            # Get ALL faces in frame (strict first, then fallback extraction).
            results = []
            try:
                ...
                logger.info(f"⚠️ Face detection strict mode failed: {str(e)[:120]}")
            ...
            if not results:
                try:
                    ...
                    for face_obj in extracted_faces:
                        ...
                        # Ignore weak pseudo-detections from fallback path.
                        if confidence < 0.70:
                            continue
                        ...
                        # DeepFace.extract_faces returns RGB float image in [0,1].
                        ...
                        if rep:
                            rep_item = rep[0]
                            rep_item["facial_area"] = facial_area
                            fallback_results.append(rep_item)

                    results = fallback_results
                    if results:
                        logger.info(f"ℹ️ Fallback face extraction used ({len(results)} face(s))")
                except Exception as fallback_error:
                    logger.info(f"⚠️ Face detection fallback failed: {str(fallback_error)[:120]}")

            if not results:
                logger.info("⚠️ Face detected nahi hua frame me (camera me face clear/front rakho)")
                return []
            ...
            for result in results:
                try:
                    frame_embedding = np.asarray(result["embedding"], dtype=np.float32)
                    ...
                    if best_match and best_similarity >= SIMILARITY_THRESHOLD:
                        ...
                        best_match["second_best_similarity"] = float(max(second_best_similarity, -1.0))
                        best_match["margin"] = float(best_similarity - max(second_best_similarity, -1.0))
                        best_match["is_confident_mark"] = (
                            (
                                best_similarity >= MARK_SIMILARITY_THRESHOLD
                                and (best_similarity - max(second_best_similarity, -1.0)) >= MIN_TOP2_MARGIN
                            )
                            or (
                                best_similarity >= RELAXED_MARK_SIMILARITY_THRESHOLD
                                and (best_similarity - max(second_best_similarity, -1.0)) >= HIGH_MARGIN_OVERRIDE
                            )
                        )
    ...
    def draw_faces_on_frame(self, frame, recognized_students):
        ...
        logger.debug(f"🎨 Drawing {len(recognized_students)} face(s) on frame")
        ...
        for student in recognized_students:
            ...
            logger.debug(f"   Drew: {name} at box")
    ...
    def mark_attendance(self, roll_number, confidence_score, current_schedule):
        ...
        # Send to backend API
        attendance_data = {
            "student_id": student.get("student_id") or f"STU_{roll_number}",
            "roll_number": roll_number,
            "camera_id": self.camera_id,
        ...
        try:
            response = requests.post(f"{BACKEND_API}/attendance", json=attendance_data, timeout=8)
            if response.status_code == 200:
                time_slot = f"{schedule.get('start_time').strftime('%H:%M')}-{schedule.get('end_time').strftime('%H:%M')}"
    ...
    def process_frame(self, frame):
        ...
        if recognized:
            ...
            for student in recognized:
                ...
                if not FORCE_MARK_ON_DETECT and not student.get("is_confident_mark", False):
                    logger.info(
                        f"   ⚠️ Skip mark (low confidence/ambiguous): "
                        f"sim={student.get('similarity', 0):.2f}, "
                        f"margin={student.get('margin', 0):.2f}, "
                        f"needs (sim>={MARK_SIMILARITY_THRESHOLD:.2f} and margin>={MIN_TOP2_MARGIN:.2f}) "
                        f"or (sim>={RELAXED_MARK_SIMILARITY_THRESHOLD:.2f} and margin>={HIGH_MARGIN_OVERRIDE:.2f})"
                    )
                    continue
                if FORCE_MARK_ON_DETECT:
                    logger.info("   ℹ️ Force mark mode active: skipping confidence gate")
